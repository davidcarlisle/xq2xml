...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="qtxslhome" select="replace($qtxhome,'XQueryX','XQueryXSLT')"/>
  ...
  <xsl:template match="/">
    <xsl:result-document href="xq2xslresults.xml">
      <test-suite-result>
...
!!<xsl:value-of select="position()"/>: <xsl:value-of select="@name"/>
		...
		<xsl:if test="not(preceding-sibling::*[1]/qt:query/@name=qt:query/@name)">
...
-->
		...
		<xsl:if test="not(../preceding-sibling::*[1]/qt:input-query/@name=@name)">
...
For this test XSLT engine used both to execute the conversion and execute the resulting stylesheet was: <xsl:value-of select="$xslt"/>.</p>
    ...
    <test-run dateRun="{format-date(current-date(), '[Y0001]-[M01]-[D01]')}">
      ...
      <transformation>
	<p>No transformations are made directly on the XQuery file, however after conversion to XSLT, the parameters
         corresponding to external $input-context variables are changed from having empty default definitions to having definition  "doc(relative/path/to/input/fileas/specified/in/catalogue)".</p>
      </transformation>
      <comparison>
